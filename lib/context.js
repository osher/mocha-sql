...
Context.prototype.config = function(args) {
    ...
    this.ciAdmin = 
      { host     : this.args.host
      , port     : this.args.port
      , user     : process.env.PSQL_CI_USER || process.env.PGUSER     || 'postgres'
      , password : process.env.PSQL_CI_PWD  || process.env.PGPASSWORD || null
      , database : process.env.PSQL_CI_DB   || process.env.PGDATABASE || 'postgres'
      }
}
